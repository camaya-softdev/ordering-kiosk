            ...
            <form method="POST" action="{{ route('locationNumber.update', $locationNumber->id) }}" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="update_ln_name">Location Number:</label>
                        <input type="text" class="form-control" id="update_ln_name" name="update_ln_name" value="{{ $locationNumber->name }}">
                    </div>

                    <div class="form-group">
                        <label for="update_location_id">Location:</label>
                        <select class="form-control" id="update_location_id" name="update_location_id">
                            @foreach($locations as $location)
                                <option id{{ $location->name }} value="{{ $location->id }}" {{ $location->id == $locationNumber->location_id ? 'selected' : '' }}>
                                    {{ $location->name }}
                                </option>
                            @endforeach
                        </select>
                                ...
                                <label class="btn btn-block btn-custom-unselected update-status-active-label">
                                    <div class="row">
                                        ...
                                        <div class="col">
                                            ...
                                            <input type="radio" name="update_ln_status" id="update_status_active_ln_{{$locationNumber->id}}" value="1" autocomplete="off">
                                ...
                                <label class="btn btn-block btn-custom-unselected update-status-inactive-label">
                                    <div class="row">
                                        ...
                                        <div class="col">
                                            ...
                                            <input type="radio" name="update_ln_status" id="update_status_inactive_ln_{{$locationNumber->id}}" value="0" autocomplete="off">
...
$(document).ready(function() {
        ...
        function setLNInitialStatus() {
            const status = "{{ $locationNumber->status }}";
            if (status === '1') {
                $('#update_status_active_ln_{{$locationNumber->id}}').prop('checked', true);
            } else {
                $('#update_status_inactive_ln_{{$locationNumber->id}}').prop('checked', true);
            }
        }

        // Call the function when the modal is shown
        $('#updateLocationNumberModal{{$locationNumber->id}}').on('shown.bs.modal', function () {
            setLNInitialStatus();
        });
